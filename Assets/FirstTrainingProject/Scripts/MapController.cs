    ...
    public class MapController : MonoBehaviour
    {
        ...
        public void CreateMap()
        {
            ...
            void MergedMaps(MapType map, MapType addMap, float percent)
            {
                for (int h = 0; h < map.Height; h++)
                {
                    ...
                    {
                        if (addMap[h, w] == (int)CellType.Marked && map[h, w] == (int)CellType.Empty && UnityEngine.Random.Range(0, (int)(1 / percent)) == 0)
                        {
                            map[h, w] = addMap[h, w];
                        }
                    }
        ...
        private void ClearEnvironments()
        {
            for (int i = _parentForEviroment.transform.childCount - 1; i >= 0; i--)
            {
                ...
                var position = _parentForEviroment.transform.GetChild(i).gameObject.transform.position;
                position.y -= 10;
        ...
        private void DrawMapCells()
        {
            ...
            for (int h = 1; h <= Map.Height - 2; h += 2)
            {
                for (int w = 1; w <= Map.Width - 2; w += 2)
                {
                    ...
                    var cellDirection = (CellDirection)(Map[h, w] % 1000) - 90; //delete - 90 and rotate models
                    ...
                    if (cellType == CellType.Branch)
                    {
                        ...
                        cellDirection = (CellDirection)(Map[h, w] % 1000);
        ...
        private void DrawMapInternalRooms()
        {
            ...
            foreach (var room in Map.InternalRooms)
            {
                ...
                var coordinates = new Vector3(w * size, height, h * size);
                var cellDirection = (CellDirection)((int)room.DoorDirection) - 90;
        ...
        private GameObject CreateEnvironmentInstance(GameObject prefab, Vector3 coordinates, CellDirection direction = CellDirection.Left)
        {
        ...
        private void Awake()
        {
            if (_parentForEviroment == null) throw new MissingFieldException($"ParentForEnviroment not set!");
            if (_enviromentPrefabsData == null) throw new MissingFieldException($"EnviromentPrefabsData not set!");
            if (_navMeshSurface == null) throw new MissingFieldException($"NavMeshSurface not set!");
